...
var registerCmd = &cobra.Command{
	Use:     "register USER [REGISTRY_ENDPOINT] [PROXY_ENDPOINT]",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) > 2 {
			...
			config_ret, err := config.Get("PROXY_ENDPOINT")
			if err != nil {
				log.Fatal("Provide a proxy endpoint or set a proxy endpoint in the digi config\n")
			}
			proxy = config_ret
		}
...
var queryCmd = &cobra.Command{
	Use:     "query USER_NAME/DSPACE/DIGI [EGRESS] [QUERY] [SOURCER_ENDPOINT]",
...
var searchCmd = &cobra.Command{
	Use:     "search [SEARCH_QUERY] [SOURCER_ENDPOINT]",
